...
def update_passenger():
    ...
    new_rating = '''            {/* STATE 4: Rating Panel Screen (Shows after ride completion) */}
            {showRating && activeRide && (
              <div className="accepted-panel animate-fade-in" style={{ background: '#ffffff', borderColor: '#e2e8f0', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: '20px', textAlign: 'center', maxHeight: '75vh', overflowY: 'auto' }}>
                ...
                {(() => {
                  const collectCashFlag = (activeRide.paymentType === 'cash' || !activeRide.paymentType);
                  const baseTotal = parseFloat(activeRide.totalKm || 8.0);
                  ...
                  return (
                    <div style={{ background: 'rgba(59, 130, 246, 0.05)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                      <h2 style={{ fontSize: '24px', fontWeight: '900', color: '#3b82f6', margin: '0 0 4px 0' }}>
                        {collectCashFlag ? 'Pay Cash' : 'Paid Online'}
                      </h2>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '13px', textAlign: 'left', background: '#ffffff', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        {tipAmount > 0 && (
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span style={{ color: 'var(--text-muted)' }}>Driver Tip</span>
                ...
                <div style={{ padding: '14px', border: '1px solid var(--border)', borderRadius: '12px', background: 'rgba(245, 158, 11, 0.03)', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
                  <h4 style={{ fontSize: '14px', fontWeight: '800', margin: 0 }}>Rate your driver</h4>
                ...
                <Button variant="primary" className="full-width" onClick={handleSubmitRating}>
                  Submit Star Feedback & Finish
                </Button>
              </div>
            )}'''
    
    pattern = re.compile(r'\{\/\*\s*STATE 4: Rating Panel Screen.*?Submit Star Feedback\s*<\/Button>\s*<\/div>\s*\)\}', re.DOTALL)
    if pattern.search(content):
        content = pattern.sub(new_rating, content)
        with open('src/pages/PassengerDashboard.jsx', 'w', encoding='utf-8') as f:
            f.write(content)
        print("Passenger Dashboard updated.")
    else:
        print("Passenger Rating pattern not found!")
