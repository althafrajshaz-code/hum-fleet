...
text = text.replace(targetDocsBtn, "");

// 2. Settle Platform Dues (Total Pending amount + remove QR code)
const targetDuesInput = `<label style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '700' }}>Enter Payment Amount (INR)</label>`;
...
text = text.replace(targetDuesInput, replaceDuesInput);

const qrRegex = /\{systemSettings\.qrCodeUrl && \([\s\S]*?Scan with GPay, PhonePe, Paytm, etc\.<\/div>\s*<\/div>\s*\)\}/g;
...
// We find the Dues Breakdown and replace it completely with the new safe version.
const duesRegex = /\{\/\*\s*Dues Breakdown\s*\*\/\}.*?Total Commission Dues:<\/span>\s*<strong[^>]*>.*?<\/strong>\s*<\/div>\s*<\/div>/s;
