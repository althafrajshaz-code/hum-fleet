...
async function run() {
  console.log("Fetching Tourist Attractions for Kerala...");
  ...
  try {
    ...
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'User-Agent': 'HumFleetDataInjector/1.2'
      }
    });
    ...
    const locations = elements.map(e => {
      let type = '';
      if (e.tags.historic) type = 'Historic ' + (e.tags.historic.charAt(0).toUpperCase() + e.tags.historic.slice(1));
      else if (e.tags.tourism) {
        const t = e.tags.tourism.replace('_', ' ');
    ...
    console.log(`Found ${uniqueLocations.length} unique, valid tourist places! Starting massive injection...`);
