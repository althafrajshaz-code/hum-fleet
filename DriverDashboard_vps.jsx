...
import { AlertCircle, Sun, Moon, Menu, Power, MapPin, Navigation, Car, AlertTriangle, ShieldCheck, DollarSign, Wallet, FileText, CheckCircle, Camera, X, Coffee, Pause, Play, MessageSquare, Send, Flame, Zap, Award, TrendingUp, Gauge, Compass, Activity, Sparkles, Settings, CreditCard, User, ChevronRight, Eye, EyeOff } from 'lucide-react';
import Button from '../components/Button';
import './Dashboard.css';

const API_BASE = import.meta.env.VITE_API_BASE || (window.location.origin.includes('localhost') && window.location.protocol !== 'capacitor:' ? 'http://localhost:5000' : window.location.origin);
...
<div class="footer">HUM Fleet Pvt Ltd • System-generated invoice • For disputes contact admin support</div>
  ...
  // --- FOREGROUND & BACKGROUND POLLING RECOVERY FIX ---
  useEffect(() => {
    let pollingInterval;
    const fetchStatus = () => {
      const email = localStorage.getItem('driverEmail');
      if (!email) return;
      fetch(`${API_BASE}/api/drivers/status?email=${encodeURIComponent(email)}`)
        .then(res => res.json())
        .then(data => {
        ...
        .catch(err => console.error('Error polling driver status:', err));
    };
    ...
    // Capacitor App State Change Listener (Resume from background)
    ...
    const setupListener = async () => {
      try {
        ...
        appStateListener = await App.addListener('appStateChange', ({ isActive }) => {
          if (isActive) {
            console.log('App returned to foreground, forcing status refresh.');
            fetchStatus();
          }
        });
      } catch (e) {
  ...
  const handleUploadProfilePic = async (file) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (res.ok) {
          alert('Captain Profile Picture updated successfully!');
          fetchStatus(false);
        }
      } catch (err) {
  ...
  const handleDriverReply = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/messages/send`, {
        ...
        body: JSON.stringify({
          driverEmail: email,
          sender: driverDetails?.name || 'Captain',
    ...
    try {
      const res = await fetch(`${API_BASE}/api/rides/messages/send`, {
        ...
        body: JSON.stringify({
          ...
          senderEmail: email,
          senderName: driverDetails?.name || 'Captain',
          ...
          <div className="driver-header-card" style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '8px', padding: '6px 0' }}>
              ...
              <div style={{ position: 'relative', width: '64px', height: '64px', borderRadius: '50%', overflow: 'hidden', border: '3px solid var(--primary)', background: 'var(--bg-main)', boxShadow: '0 0 0 4px var(--bg-card)', margin: '0 auto' }}>
                {driverProfilePic ? (
                  <img src={driverProfilePic} alt="Captain Profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '800', color: 'var(--text-main)' }}>
                  {driverDetails?.name || 'Captain'}
                </h2>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginTop: '4px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>HUM Fleet Partner</span>
                      ...
                      {availablePreBooked.map(ride => (
                        <div key={ride.id} style={{ border: '1px solid var(--border)', borderRadius: '14px', padding: '14px', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--text-muted)' }}>
                            ...
                            <span>📍 Captain Dist: <strong>{ride.distance} KM</strong></span>
                          </div>
            ...
            <div className="tab-pane animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: '14px', padding: '16px', background: 'rgba(255, 255, 255, 0.01)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: '800' }}>
                  <Wallet size={18} color="var(--primary)" /> Captain Financial Ledger & Wallet
                </span>
            ...
            {homeEarnings ? (() => {
              ...
              return (
                <>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {period.rides.map((ride, i) => (
                          <div key={ride.id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '10px' }}>
                            ...
                            <div style={{ textAlign: 'right', marginLeft: '12px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                  ...
                  <button
                    onClick={() => {
                      const driverName = driverDetails?.name || 'Captain';
