...
async function run() {
  console.log("Fetching Malls, Hospitals, Temples, Churches, and Mosques for Kerala...");
  ...
  try {
    ...
    console.log(`Received ${elements.length} named locations from Overpass.`);

    // Format them for our database
    const locations = elements.map(e => {
      let type = '';
      if (e.tags.shop === 'mall') type = 'Mall';
      else if (e.tags.amenity === 'hospital' || e.tags.amenity === 'clinic') type = 'Hospital/Clinic';
      else if (e.tags.religion === 'hindu') type = 'Temple';
      else if (e.tags.religion === 'christian') type = 'Church';
      else if (e.tags.religion === 'muslim') type = 'Mosque';
    ...
    const chunkSize = 50;
    for (let i = 4000; i < uniqueLocations.length; i += chunkSize) {
